...
private:
	...
	///
	///	Auxiliar constants to control the state of the parker
	///
#define LOCK_BIT  (0)
...
public:
	...
	NOINLINE ParkerStatus ParkThread(U32 timeout)
	{
		...
		///
		BaseUScheduler<Context>::RemoveThreadFromReadyQueue(*this);
	...
	NOINLINE Void UnparkThread(ParkerStatus status = Success)
	{
		...
		if(!TestAndClearMask(PARK_IN_PROGRESS_MASK))
		{
			...
			BaseUScheduler<Context>::InsertThreadInReadyQueue(*this);
